            ...
            .when('serverMsgType', serverMsgTypes.fbUpdate, function (vars) {
                ...
                .repeat('nRects', function (vars, i) {
                    this.into('rect', function () {
                        ...
                        /*
                        .when('encodingType', encodings.rre) {
                            sys.log('rre encoding!');
                            this
                            .tap(function (vars) { vars.emitter = 'rre' })
                        })
                        .when('encodingType', encodings.hextile, function (vars) {
                            var tiles_x = Math.ceil(vars.rect.width/16);
                            var tiles_y = Math.ceil(vars.rect.height/16);
                            this.repeat(tiles_x, function (vars, x_tile) {
                                this.repeat(tiles_y, function (vars, y_tile) {
                                    var x = x_tile*16, y = y_tile*16;
                                    sys.log(x + ' ' + y);
                                })
                            });
                            sys.log('hextile encoding!')
                            this
                            .tap(function (vars) { vars.emitter = 'hextile' })
                            .getWord8('subencodingMask')
                            .getWord32be('bgSpec')
                            .getWord32be('fgSpec')
                            .getWord32be('nSubrects')
                            .tap(function (vars) {
                                sys.log(vars.subencodingMask);
                                sys.log(vars.bgSpec);
                                sys.log(vars.fgSpec);
                                sys.log(vars.nSubrects);
                            })
                        })
                        */
