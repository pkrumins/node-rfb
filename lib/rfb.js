...
function RFB(opts) {
    ...
    this._dimQueue = []; // dimension buffering hack
    this.dimensions = function (f) {
        this._dimQueue.push(f);
    };
...
function Parser (rfb, bufferList) {
    Binary(bufferList)
        ...
        .tap(function (vars) {
            ...
            rfb._dimQueue.forEach(function (f) { f(rfb.frameBuffer) });
            delete rfb._dimQueue; // hack-be-gone
            rfb.dimensions = function (f) { f(rfb.frameBuffer) };
                ...
                .repeat('nRects', function (vars, i) {
                    ...
                    this.into('rect', function (rect) {
                        ...
                        .when('encodingType', encodings.pseudoDesktopSize, function (rect) {
                            rfb.frameBuffer.width = rect.width;
                            rfb.frameBuffer.height = rect.height;
